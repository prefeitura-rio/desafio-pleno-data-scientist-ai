...
def run_agent_hface(huggingface_api_key, bigquery_credentials_path):
    ...
    print("Agente GenAI (Hugging Face) iniciado. Pergunte sobre dados públicos do Rio de Janeiro.")
    while True:
        ...
        if not sql_query or sql_query.strip().lower().startswith("erro") or not sql_query.strip().lower().startswith("select"):
            print(f"Não foi possível gerar uma consulta SQL válida. Resposta do modelo: {sql_query}")
